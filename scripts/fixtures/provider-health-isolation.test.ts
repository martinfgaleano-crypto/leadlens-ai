...
async function main(){
  ...
  await test("Anthropic low credit 400 is exhausted, not unknown",async()=>{global.fetch=async(input)=>{calls.push(String(input));return new Response(JSON.stringify({type:"error",error:{type:"invalid_request_error",message:"Your credit balance is too low to access the Anthropic API"}}),{status:400});};const status=await probeOne("anthropic",true);assert.equal(status?.state,"exhausted");assert.match(status?.detail??"",/límite de uso/i);});
