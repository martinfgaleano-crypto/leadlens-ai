...
const pilotPage=readFileSync("app/admin/pilot/page.tsx","utf8"),workspacePage=readFileSync("app/admin/intelligence/pilots/[pilotId]/page.tsx","utf8")+readFileSync("app/admin/intelligence/pilots/[pilotId]/pilot-experience.tsx","utf8"),api=readFileSync("app/api/admin/intelligence/pilots/[pilotId]/intake/route.ts","utf8"),core=readFileSync("lib/intelligence/pilot-workspace.ts","utf8");
[
 ...
 ["5 partial not blank",!!w.availability.message&&!!w.overview.diagnosis],["6 DB accepted explicit",w.availability.database==="available"],
 ...
 ["16 submit review",api.includes("review_required")],["17 accepted context explicit",w.pilot.active_context_version==="context_28bbc2b447323da3e387c964"],["18 history preserved",!api.includes('.from("intelligence_client_intakes").update(')],
 ...
 ["26 unknown not zero",workspacePage.includes("Aún no medido")],["27 feasibility resolves",w.feasibility.every((x:any)=>x.dimensions.every((d:any)=>d.next_verification))],
 ["28 section independent",new Set(w.sections.map((x:any)=>x.state)).size>1],["29 report disabled",w.final_report_generation==="disabled"&&workspacePage.includes("reporte final permanecerá deshabilitado")],
 ...
 ["38 Phase2 context truth",w.questions.length===17&&w.overview.context_completeness===100],["39 decisions",w.overview.decisions.prioritize===4&&w.overview.decisions.monitor===2],
 ["40 entities",w.accounts.every((x:any)=>x.identity)],["41 zero signals",w.overview.signals===0],["42 evidence honest",w.overview.evidence_coverage.includes("baseline account evidence remains unchanged")],
 ...
 ["48 premium sections",["brief","portfolio","account","readiness","evidence","report"].every(x=>workspacePage.includes(`id="${x}"`))],
