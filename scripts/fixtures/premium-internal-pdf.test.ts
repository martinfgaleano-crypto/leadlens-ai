...
import { buildInternalPilotPdf, PDF_MIN_BODY_SIZE, PREMIUM_EXPECTED_PAGES, PREMIUM_PDF_VERSION, premiumAccountEditorial } from "@/lib/reports/internal-pilot-pdf";
...
[
  ...
  ["04 premium cover exists", source.includes("Commercial Opportunity") && source.includes("INTERNAL INTELLIGENCE BRIEF")],
  ["05 Executive Brief exists", source.includes('addPage("Executive Brief")')],
  ["06 key decisions exists", source.includes('addPage("Decisiones")')],
  ["07 ICP exists", source.includes('addPage("Perfil de cliente ideal")')],
  ["08 visual market funnel exists", source.includes("const funnel") && source.includes("roundedRect(x, y, w, 18")],
  ["09 recommended portfolio exists", source.includes('addPage("Portafolio recomendado")')],
  ["10 portfolio map exists", source.includes('addPage("Mapa de portafolio")') && source.includes("const positions")],
  ["11 all six account pages exist", accounts.length === 6 && source.includes("8-13 - Account pages")],
  ["12 distinct account theses", distinct("thesis")],
  ["13 distinct account use cases", distinct("useCase")],
  ["14 distinct account actions", distinct("action")],
  ["15 distinct account triggers", distinct("trigger")],
  ["16 client-context priorities exist", source.includes("Cinco preguntas de mayor impacto")],
  ["17 timing interpretation exists", source.includes("Cero senales no significa cero oportunidad")],
  ["18 report readiness exists", source.includes("Preparacion del reporte") && source.includes("BASE SOLIDA")],
  ["19 methodology exists", source.includes('addPage("Metodologia y limites")')],
  ["20 limitations exist", source.includes("Limitaciones del piloto")],
  ["21 four claim labels exist", ["HECHO", "INFERENCIA", "RECOMENDACION", "LIMITACION"].every(label => source.includes(label))],
  ...
  ["29 PDF metadata populated", source.includes("setProperties") && source.includes('author: "LeadLens"') && PREMIUM_PDF_VERSION.includes("v2")],
