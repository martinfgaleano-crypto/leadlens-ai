...
async function run() {
  ...
  test("38 latest bounded capture evidence is reflected without overstating validation",
    dynamicDiscovery?.state === "soak_validated" && dynamicDiscovery.supporting_metrics.positive_controls_captured === 7 && dynamicDiscovery.blockers.some((b) => /no customer-safe Case has been human-confirmed/i.test(b)));
