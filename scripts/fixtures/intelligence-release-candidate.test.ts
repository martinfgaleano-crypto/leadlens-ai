...
test("28 canonical Hold removes contradictory confirmed-now prose from customer narrative", () => {
  const l = lead({ enrichment: { account_research: telemetry({ validated_events: [], early_stop_reason: "no_material_event" }), why_now: "Confirmed: a new plant creates an active buying trigger right now.", recommended_action: "send_outreach_now" } }) as unknown as { enrichment: { why_now?: string; recommended_action?: string } };
  ...
  assert.equal(l.enrichment.recommended_action, "exclude");
});
...
console.log(`\n${passed} intelligence release-candidate contracts passed`);
