...
const DECISIONS: D[] = [
  ...
  // Apex B2B downgraded on re-validation: source freshness_status=stale (2026-04-09),
  // stored qualification has qualified_opportunity=false, category-like name derived
  // from headline (title_prefix, medium confidence), region metadata wrong (stored US,
  // evidence is UK/£1.3m/UKTN), single source. Fails approve gates — monitor only.
  { sig: "9f5b65aa", decision: "approved_monitor_only", tier: "C", rights: "internal_only", conf: 0.6, reasons: ["grounded_claim", "stale_signal", "monitor_only"], note: "Apex B2B: real dated launch (£1.3m, UKTN 2026-04-09) but stale per source freshness, qualified_opportunity=false in stored evidence, category-like headline-derived name and US/UK region mismatch — not customer-approvable on this evidence.", verdicts: { company_match: false, date_valid: true, claim: true, signal: true } },
