...
const CONTENT: Record<LandingLocale, { eyebrow: string; hint: string; lenses: Record<Lens, { label: string; title: string; body: string; trace: string; outcome: string }> }> = {
  en: { eyebrow: "Explore the decision", hint: "Choose a lens", lenses: {
  ...
  es: { eyebrow: "Explora la decisión", hint: "Elige una perspectiva", lenses: {
  ...
  pt: { eyebrow: "Explore a decisão", hint: "Escolha uma perspectiva", lenses: {
  ...
  ja: { eyebrow: "意思決定を探索", hint: "視点を選択", lenses: {
...
const ORDER: Lens[] = ["prioritize", "why", "evidence", "compare", "validate", "monitor"];
...
export function CommercialIntelligenceExplorer({ locale }: { locale: LandingLocale }) {
  ...
  return <div className={styles.shell} aria-label={c.eyebrow}>
    <div className={styles.top}><span>{c.eyebrow}</span><small>{c.hint}</small></div>
    <div className={styles.stage} data-lens={selected}>
      <div className={styles.orbit} aria-hidden="true"><i /><i /><i /></div>
      <div className={styles.controls}>
        {ORDER.map((lens, index) => <button key={lens} className={selected === lens ? styles.selected : ""} aria-pressed={selected === lens} onClick={() => setSelected(lens)} style={{ "--i": index } as React.CSSProperties}><span>{c.lenses[lens].label}</span></button>)}
      </div>
      <article className={styles.focus} aria-live="polite" key={selected}>
        <div className={styles.decisionLine}><span>LeadLens</span><b>{active.label}</b></div>
        <h2>{active.title}</h2><p>{active.body}</p>
        <div className={styles.proof}><span>{active.trace}</span><strong>{active.outcome}</strong></div>
      </article>
      <div className={styles.markers} aria-hidden="true"><span /><span /><span /><span /></div>
    </div>
  </div>;
}
