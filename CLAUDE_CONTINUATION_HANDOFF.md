...
## Discovery Engine V2.1 — live validation (2026-08-04)
- First REAL controlled source access (Cotelco afiliados via in-app browser). New lib/discovery/source-intelligence/live.ts + artifact output/discovery-v2-colombia-hospitality-live-001.json (data_basis=live_source, live_execution=true, provider_calls=0, n=4). Fixture artifact -001.json UNCHANGED (regression).
- REAL findings: Cotelco afiliados is public + structured + provides official domains DIRECTLY (4/4) — contradicts the fixture assumption (RNT-based) that structured sources lack domains; real blocker is JS-driven non-URL-addressable pagination (scale). Search cohort not_executed (no provider creds in exec env + exhaustion) — recorded operational limitation. Entities company-level only (no people data, §48).
- co_cotelco: hypothesized→benchmarked (n=4, small; NOT historically_effective). Learning approval-gated. Strategy ranking = INSUFFICIENT LIVE EVIDENCE. Manufacturing NOT YET.
- Observatory /admin/intelligence/discovery has a separate LIVE section (accessibility, verification economics, research depth L0–L5, fixture-vs-live). test:discovery-engine-v2-1-live 25/0; regressions green; tsc + build clean. Commit `feat: run live controlled discovery validation for colombia hospitality`. Next: build Cotelco JS parser + run search cohort under a real key. Report: LEADLENS_DISCOVERY_ENGINE_V2_1_LIVE_VALIDATION_REPORT.md.


## Discovery V2.4 — multi-country foundation + Colombia retail (2026-08-07)
- Resumed Codex V2.2/V2.3 (coverage.ts atlas + manufacturing-live.ts) — extended, not rebuilt. New lib/discovery/source-intelligence/{multi-country,retail-live}.ts.
- Five markets (CO/US/GB/AU/CA): CountryProfiles, Foundation Source concept (isFoundationSource), curated atlases US5/UK4/AU3/CA3 (confidence discovered/hypothesized, accessibility researched_not_tested — NOT live-validated), generic geography/identifiers/terminology, source layers, legal-reuse+personal-data+db-right flags, five-country readiness (depth untested outside CO), per-country benchmark+research queues (planned, not executed; US=priority 1), Market Memory + cross-country Source Type Learning (hypotheses only).
- Colombia Retail benchmark #3 (discovery-v2-colombia-retail-live-001, controlled_sample, 0 providers): Location Inflation Ratio 2.13 (17→8 canonical), marketplace classification, distributor≠retail, assortment yield 1.0, listing-vs-account saturation, chain concentration, source bias (Bogotá/chain/marketplace). Generalization status = moderate_adjustments_required; reuses for franchises/networks.
- Observatory /admin/intelligence/discovery: five-market home + retail section (above preserved CO/live/fixture). No fabricated %; depth untested outside CO.
- test:discovery-engine-v2-4-multi-country 48/0; regressions (manufacturing 62, coverage 46, live 25, fixture 40, source-intel 38, recurring 50, pilot1 ok); tsc+build green. No migration. Commit `feat: expand discovery to five priority markets`.
- Providers still not loaded in exec shell → live cohorts blocked. Next: Colombia Technology #4 + USA foundation validation (needs a real provider key). Report: LEADLENS_DISCOVERY_V2_4_MULTI_COUNTRY_FOUNDATION_AND_RETAIL_REPORT.md.


## Discovery V2.4.1 — live recovery + foundation validation (2026-08-07)
- INTEGRITY FIX: retail controlled sample was mislabeled `discovery-v2-colombia-retail-live-001`; renamed to `discovery-v2-colombia-retail-controlled-001`; `-live-001.json` replaced by a reclassification stub reserving the live id; retail depth = controlled_sample_complete·live_benchmark_pending (NOT benchmarked; CO live-depth stays 2).
- PROVIDER ENV ROOT CAUSE: tsx CLI scripts don't auto-load .env.local — must call loadEnvConfig(process.cwd()). New lib/discovery/source-intelligence/provider-env.ts (loadCliEnv + providerEnvDiagnostic, booleans only). Diagnostic: all 4 keys before_load=false → after_load=true (fixed). 0 billable calls (call_tested=false).
- REAL foundation validation via browser (0 providers): UK Companies House, AU ABN Lookup, CA Corporations Canada (federal), US SEC EDGAR → operationally_validated. All: identity yes, domain NO, account-first feasible. USA fragmented (SEC=4 public cos vs 714 CA-federal vs >200 AU); CA=federal_plus_provincial. lib/discovery/source-intelligence/foundation-validation.ts + empiricalReadiness (depth=live benchmarks only; GB/AU validated but depth 0 = validation≠benchmark).
- Observatory: Foundation Validation table + Retail=CONTROLLED banner. test:discovery-engine-v2-4-1-live-recovery 34/0; all discovery regressions green; tsc+build clean; no migration. Commit `feat: validate retail and international discovery foundations`.
- STILL BLOCKED: live retail + USA state validation need an authorized search-provider key/quota (keys now visible to runner, but no billable calls made). Next: V2.5 Retail LIVE + USA foundation state validation. Report: LEADLENS_DISCOVERY_V2_4_1_LIVE_RECOVERY_AND_FOUNDATION_VALIDATION_REPORT.md.
