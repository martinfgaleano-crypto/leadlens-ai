...
import { listVaultCompanies, createVaultCompany } from "@/lib/storage/vault-store";
...
export async function GET(req: NextRequest) {
  ...
  if (deny) return deny;
  const items = await listVaultCompanies();
...
export async function POST(req: NextRequest) {
  ...
  const created = await createVaultCompany(body);
