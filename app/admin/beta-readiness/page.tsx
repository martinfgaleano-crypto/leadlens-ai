...
type Payload = {
  ...
  production_configuration: Array<{ id: string; label: string; state: string; launch_stage: string; detail: string; hard_blocking_readiness: boolean; founder_action_required: boolean }>;
  ...
  return <AdminLayout><main style={{ maxWidth: 1120, margin: "0 auto", color: "#0f172a" }}>
    ...
    {r && <>
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 16, marginBottom: 18 }}>
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20 }}><div style={{ color: "#64748b", fontSize: ".72rem", fontWeight: 700 }}>INTELLIGENCE SCORE</div><div style={{ color: "#0369a1", fontWeight: 900, fontSize: "3.2rem", lineHeight: 1.1, marginTop: 8 }}>{data.capability_summary.overall.state === "measured" ? data.capability_summary.overall.score : "—"}<span style={{ fontSize: "1rem" }}>/100</span></div><div style={{ color: "#64748b", fontSize: ".75rem", marginTop: 4 }}>Engine quality · confidence {data.capability_summary.confidence}</div><div style={{ color: "#475569", fontSize: ".72rem", marginTop: 8 }}>Separate from launch safety and production configuration.</div></div>
        ...
        <div style={{ background: "#0f172a", color: "#e2e8f0", borderRadius: 12, padding: 20 }}><h2 style={{ color: "#fff", margin: "0 0 .6rem", fontSize: "1rem" }}>Current launch truth</h2>{r.blockers.length ? <ul style={{ paddingLeft: 18, margin: 0 }}>{r.blockers.map((item) => <li key={item} style={{ marginBottom: 7, lineHeight: 1.45 }}>{item}</li>)}</ul> : <p>No failing gate in the current evidence.</p>}<p style={{ color: "#94a3b8", fontSize: ".72rem", margin: "1rem 0 0" }}>Evaluated {new Date(r.evaluated_at).toLocaleString()} · source cutoff {r.source_data_cutoff ? new Date(r.source_data_cutoff).toLocaleString() : "unavailable because no measured evidence source is available"}</p></div>
      </section>
      ...
      <section style={{ marginBottom: 18, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 18 }}><h2 style={{ fontSize: "1rem", marginTop: 0 }}>Production configuration</h2><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 10 }}>{data.production_configuration.map((check) => <div key={check.id} style={{ border: "1px solid #e2e8f0", borderRadius: 8, padding: 10 }}><strong style={{ fontSize: ".78rem" }}>{check.label}</strong><div style={{ color: check.state === "present" ? "#15803d" : check.state === "missing" || check.state === "invalid" ? "#b91c1c" : "#a16207", fontSize: ".7rem", fontWeight: 800, textTransform: "uppercase", margin: "3px 0" }}>{check.state}</div><div style={{ color: "#64748b", fontSize: ".7rem" }}>{check.detail} Affects {words(check.launch_stage)} · {check.hard_blocking_readiness ? "hard readiness blocker" : "stage-scoped degradation"}{check.founder_action_required ? " · founder action required" : ""}.</div></div>)}</div></section>
      ...
      <section style={{ marginTop: 22, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 18 }}><h2 style={{ fontSize: "1rem", marginTop: 0 }}>Durable history</h2>{data.history_summary.state === "insufficient_history" && <p style={{ color: "#64748b", fontSize: ".8rem" }}>One baseline exists. History is insufficient for a trend; no delta is inferred.</p>}{data.history_summary.state !== "insufficient_history" && <p style={{ color: "#334155", fontSize: ".8rem" }}>Readiness delta: <strong>{data.history_summary.readiness_delta! >= 0 ? "+" : ""}{data.history_summary.readiness_delta}</strong> · Intelligence delta: <strong>{data.history_summary.capability_delta ?? "unmeasured"}</strong> · {data.history_summary.gate_transitions.length} gate transitions.</p>}{data.history.length ? <div style={{ overflowX: "auto" }}><table style={{ borderCollapse: "collapse", width: "100%", fontSize: ".76rem" }}><thead><tr>{["Observed", "Readiness", "Level", "Intelligence", "Confidence", "Blockers"].map((x) => <th key={x} style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #cbd5e1" }}>{x}</th>)}</tr></thead><tbody>{data.history.map((x) => <tr key={`${x.observed_at}:${x.score}`}><td style={{ padding: 8 }}>{new Date(x.observed_at).toLocaleString()}</td><td>{x.score}</td><td>{words(x.level)}</td><td>{x.capability_score ?? "unmeasured"}</td><td>{x.confidence}</td><td>{x.blocker_count}</td></tr>)}</tbody></table></div> : <p style={{ color: "#64748b", fontSize: ".8rem" }}>No persisted history is available. This is an honest empty state, not a zero trend.</p>}</section>
