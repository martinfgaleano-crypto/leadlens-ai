...
const HIGH_NETWORK_EXPANSION = /\bexpand(?:ed|s|ing) (?:its |the )?(?:(?:service logistics|distribution|fulfillment|forward stocking|operational) )?(?:network|capabilities)\b[\s\S]{0,180}?\b(?:\d{2,}|national|nationwide|multiple|major)\b|\b(?:\d{2,}|national|nationwide|multiple|major)\b[\s\S]{0,180}?\bexpand(?:ed|s|ing) (?:its |the )?(?:(?:service logistics|distribution|fulfillment|forward stocking|operational) )?(?:network|capabilities)\b/i;
const HIGH_ACTIVE_EXPANSION = /\b(?:is|are) expanding (?:its |their )?(?:capacity|footprint|production|(?:[a-z-]+ ){0,2}operations)\s+(?:with|through) (?:a |an |the )?(?:new |additional )?(?:plant|facility|factory|warehouse|distribution cent(?:er|re)|manufacturing facility|production line)\b/i;
const HIGH_COMMITTED_INVESTMENT = /(?:\b(?:approximately |about |more than )?(?:us\$|\$)\s?[\d.,]+\s*(?:million|billion|m|bn)?\s+investment\s+will\s+(?:add|create|expand|increase)|\bwill invest\s+(?:approximately |about |more than )?(?:us\$|\$)\s?[\d.,]+|\bexpand(?:s|ing)\b[^.]{0,140}\bwith\s+(?:an? )?(?:us\$|\$)\s?[\d.,]+\s*(?:million|billion|m|bn)?\s+investment|\bannounced plans to expand\b[\s\S]{0,320}?\binvestment of (?:approximately |about |more than )?(?:us\$|\$)\s?[\d.,]+\s*(?:million|billion|m|bn)?)/i;
const HIGH_CONCRETE_COMMITMENT = /\b(?:breaks?|broke) ground on (?:a |an |the )?(?:major |new |largest )?(?:expansion|plant|facility|factory|warehouse|distribution cent(?:er|re))\b|\bannounced plans to (?:open|expand)[^.]{0,180}\b(?:new |additional |two |three |\d+ |\$\s?[\d.,]+|investment|facilit(?:y|ies)|plants?|factories|distribution cent(?:er|re)s?)\b|\bannounced (?:a |an |the )?(?:new |major )?(?:manufacturing )?(?:plant|facility|factory|warehouse|distribution cent(?:er|re))\b|\bincreased (?:its )?planned [^.]{0,100}investment to \$\s?[\d.,]+\s*(?:million|billion|m|bn)?\b|\blaunch(?:ed|es) [^.]{0,100}(?:supply chain|logistics) services\b[^.]{0,180}\bopen(?:ed|ing) (?:its |the )?[^.]{0,80}\bnetwork\b|\bis expanding (?:its )?(?:[a-z-]+ ){0,3}operations with (?:a |an |the )?(?:new |additional )?(?:[\d,]+-square-foot |[a-z-]+ ){0,3}(?:plant|facility|factory|warehouse|distribution cent(?:er|re))\b|\bopen(?:ed|s) a new station\b[^.]{0,120}\b(?:\d+ dedicated routes|operational capacity)\b|\bp(?:u|ú)so(?:ieron)? en operaci(?:o|ó)n (?:una |un )?nuev[ao] (?:planta|unidad|centro)\b|\bconstruyeron? (?:dos|tres|cuatro|\d+) (?:nuevas? )?(?:l[ií]neas|instalaciones|plantas)[^.]{0,120}\b(?:mw|capacidad|operaci[oó]n)\b/i;
...
export function classifyMateriality(titleAndContent: string): { level: Materiality; matched: string | null } {
  ...
  const h = hay.match(HIGH) ?? hay.match(HIGH_VARIANTS) ?? hay.match(HIGH_DESCRIPTIVE_OPEN) ?? hay.match(HIGH_NETWORK_EXPANSION) ?? hay.match(HIGH_ACTIVE_EXPANSION) ?? hay.match(HIGH_COMMITTED_INVESTMENT) ?? hay.match(HIGH_CONCRETE_COMMITMENT);
