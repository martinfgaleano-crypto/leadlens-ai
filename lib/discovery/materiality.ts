...
export function classifyMateriality(titleAndContent: string): { level: Materiality; matched: string | null } {
  ...
  const h = hay.match(HIGH) ?? hay.match(HIGH_VARIANTS) ?? hay.match(HIGH_DESCRIPTIVE_OPEN) ?? hay.match(HIGH_NETWORK_EXPANSION) ?? hay.match(HIGH_ACTIVE_EXPANSION) ?? hay.match(HIGH_COMMITTED_INVESTMENT);
