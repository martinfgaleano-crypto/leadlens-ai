...
// Corporate history/timeline pages can contain dozens of genuine past changes
// plus a relative word such as "today". They are useful identity/context sources,
// but the page's current publication/update date must never re-date that history
// into a fresh event. A concrete newsroom headline is intentionally not covered.
const HISTORICAL_REFERENCE_PAGE = /^(?:[^.]{0,120}\b(?:our|company|corporate) history\b|[^.]{0,120}\bhistory\s*[-–—:]|[^.]{0,120}\b(?:our )?timeline\b)/i;
...
export function classifySignalKind(titleAndContent: string): { kind: SignalKind; matched: string | null; can_trigger: boolean } {
  ...
  const historicalPage = hay.match(HISTORICAL_REFERENCE_PAGE);
  if (historicalPage) return { kind: "reference_information", matched: historicalPage[0], can_trigger: false };
