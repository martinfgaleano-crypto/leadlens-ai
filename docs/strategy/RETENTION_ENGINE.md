...
**Production Sprint v0 (2026-07-02):** la promesa de recurrencia ahora tiene infraestructura self-healing: runs perdidos/stalled se recuperan solos (drainer + cron) o con un click (ops center) — un beta customer nunca queda esperando un reporte que no llega sin que el admin lo vea. Ver BETA_OPERATIONS_PLAYBOOK.md para la operación mensual manual mientras no hay scheduler.

**Reliability Sprint v0 (2026-07-02):** los monitor runs son asíncronos — el cliente recibe respuesta en segundos y la UI muestra progreso real (processing → report ready), con recovery de runs stalled. La promesa de recurrencia mensual ya no depende de que un request sobreviva minutos de pipeline. Ver ASYNC_RUN_EXECUTION.md.
