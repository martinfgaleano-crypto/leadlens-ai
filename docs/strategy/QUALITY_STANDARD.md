...
- **Production Sprint v0 (2026-07-02):** self-healing operativo — drainer bounded (stale-only, supersede, abandono a 6h) con cron diario y ops center admin (`/admin/monitor-runs`); env health en system-health (production_safe); fundaciones inertes de scheduler/billing/source engine con reglas explícitas anti-scraping y anti-datos-personales a nivel de contrato de tipos. Smoke steps 34–45; checklist de deploy y playbook de operación beta.
- **Reliability Sprint v0 (2026-07-02):** ejecución async de runs (`ASYNC_RUN_EXECUTION.md`): rutas responden 202, processor interno secret-protected ejecuta el pipeline, jobs stale detectables (STALLED) y recuperables vía admin retry sin duplicar runs frescos. Estados `stale_processing` en lifecycle/readiness con copy consistente customer/admin. Smoke steps 24–33.
- **Self-Serve Foundation Sprint v0 (2026-07-02):** `POST /api/monitor/[id]/run` customer-triggered con los MISMOS guards del path admin (ownership, onboarding, dedup, más entitlement). `LeadLensReport.search_id` como contexto (mismatch snapshot/report se loguea y se oculta a no-admins). Lifecycle states unificados (`lib/monitor/lifecycle.ts`) con copy customer-safe. QA admin advisory — `needs_review` no bloquea al customer (decisión registrada en REPORT_ACCESS_MODEL.md). Smoke helper read-only: `npm run smoke:selfserve`.
- **Beta-Ready Product Sprint v0 (2026-07-02):** dashboard customer sin campos de contacto (tabla + CSV account-level); `GET /api/report` con ownership (snapshot-first, legacy admin-only, 404 para no-owners); feedback dedup por job_id+company+signal; readiness verdict por serie en admin (READY TO REVIEW / REVIEW RECOMMENDED / FAILED / PROCESSING / NO RUNS / SETUP INCOMPLETE); report v2 con strips de evidencia y freshness (unknown siempre visible como unknown). Superficies mapeadas en `BETA_READINESS_SURFACE_MAP.md`; smoke QA en `BETA_SMOKE_QA.md`; modelo de acceso en `REPORT_ACCESS_MODEL.md`. La deuda de la tabla legacy del dashboard quedó resuelta en P1 de este sprint.
- **Customer report page account-level (SaaS Readiness Sprint v0):** `/results/[jobId]` reescrito sin datos de contacto personales — las mismas reglas de copy de exports (baseline vs comparación, client_visible, fallbacks "Not available"/"Signal date not confirmed") aplican en la página. Feedback account-level integrado por card. Sección Monthly Monitor en el dashboard del cliente (run history con ownership check). QA flags automáticos por run en admin (evidencia débil dominante, repetición dominante, run failed) + Product Readiness QA Checklist v0 (sección abajo). **Deuda conocida:** la tabla de leads legacy del dashboard (`lead_results` del flow Apollo) aún muestra contactos — decisión de producto pendiente, ver DECISION_LOG.
